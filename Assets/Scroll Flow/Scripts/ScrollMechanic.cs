...
public class ScrollMechanic : MonoBehaviour, IDropHandler, IDragHandler, IBeginDragHandler, IPointerExitHandler, IPointerEnterHandler
{
    ...
    public float minVelocity = 0.2f; // Minimum inertia value to start concentrating
    ...
    public float maxFontSize = 48.2f; // Maximum font size

    public bool isElastic = true; // Is elastic movement
    public float maxElastic = 50; // Maximum elasticity distance
    ...
    [Header("Identifier")]
    public string identifier; // Unique identifier for each scroll mechanic
    ...
    public void Initialize(List<string> dataToInit, bool isInfinite = false, int firstTarget = 0)
    {
        ...
        if (isInfinite)
        {
            ...
            if (dataToInit.Count > half)
            {
                ...
                {
                    var textComponent = Instantiate(templateValues, contentTarget.transform).transform.GetChild(0).GetComponent<TextMeshProUGUI>();
    ...
    private void Update()
    {
        ...
        {
            isDragging = false;
            SaveCurrentSelection(); // Save selection when mouse button is released
        }
    ...
    {
        string selectedText = contentTarget.GetChild(currentCenter).GetChild(0).GetComponent<TextMeshProUGUI>().text;
        PlayerPrefs.SetString(identifier + "_SelectedOption", selectedText);
        PlayerPrefs.Save();
    }
